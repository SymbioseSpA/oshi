...
public class LinuxGlobalMemory implements GlobalMemory {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(LinuxGlobalMemory.class);
	...
	public long getAvailable() {
		...
		try {
			memInfo = FileUtil.readFile(PROC_MEMINFO);
		} catch (IOException e) {
		...
		if (0 == _totalMemory) {
			...
			try {
				...
				if (0 != Libc.INSTANCE.sysinfo(info)) {
					LOG.error("Failed to get total memory. Error code: "
							+ Native.getLastError());
				} else {
					_totalMemory = info.totalram.longValue() * info.mem_unit;
					return _totalMemory;
				}
			} catch (UnsatisfiedLinkError | NoClassDefFoundError e) {
				LOG.warn(
						"Failed to get total memory from sysinfo. Falling back /proc/meminfo MemTotal. {}",
						e.getMessage());
			}
			...
			for (String checkLine : memInfo) {
				if (checkLine.startsWith("MemTotal:")) {
					String[] memorySplit = checkLine.split("\\s+");
					_totalMemory = parseMeminfo(memorySplit);
					break;
				}
			}
		}
	...
	public JsonObject toJSON() {
		...
		return NullAwareJsonObjectBuilder
				...
				.add("available", getAvailable()).add("total", getTotal())
				.build();
	}
}
